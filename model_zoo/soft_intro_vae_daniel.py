...
Author: Tal Daniel
Code from: https://github.com/taldatech/soft-intro-vae-pytorch/blob/main/soft_intro_vae/
...
class Encoder(nn.Module):
    def __init__(self, nc=3, zdim=512, channels=(64, 128, 256, 512, 512, 512), image_size=256, conditional=False,
                 cond_dim=10):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(nc, cc, 5, 1, 2, bias=False),
            nn.BatchNorm2d(cc),
            nn.LeakyReLU(0.2),
            nn.AvgPool2d(2),
        )
    ...
    def calc_conv_output_size(self):
        dummy_input = torch.zeros(1, self.nc, self.image_size, self.image_size)
    ...
    # Added
    def initialization_(self, init_type='xavier'):
        """
...
class Decoder(nn.Module):
    def __init__(self, nc=3, zdim=512, channels=(64, 128, 256, 512, 512, 512), image_size=256, conditional=False,
        ...
        self.main.add_module('res_in_{}'.format(sz), ResidualBlock(cc, cc, scale=1.0))
        self.main.add_module('predict', nn.Conv2d(cc, nc, 5, 1, 2))
...
class SoftIntroVAE(nn.Module):
    def __init__(self, nc=1, zdim=128, channels=(64, 128, 256, 512, 512), image_size=128, conditional=False,
                 cond_dim=10):
        ...
        self.encoder = Encoder(nc, zdim, channels, image_size, conditional=conditional, cond_dim=cond_dim)

        self.decoder = Decoder(nc, zdim, channels, image_size, conditional=conditional,
    ...
    def forward(self, x, o_cond=None, deterministic=False):
        if self.conditional and o_cond is not None:
            ...
            if deterministic:
                ...
                z = reparameterize(mu, logvar)
            y = self.decode(z)
        return y, {'z_mu': mu, 'z_logvar': logvar,'z': z}
...
def calc_reconstruction_loss(x, recon_x, loss_type='mse', reduction='sum', annealing = 0, device = None):
    """
    ...
    #tmp_recon_x = recon_x.to(device)
    #tmp_x = x.to(device)

    if loss_type == 'pl':
        loss_type = 'mse' # pl loss added in trainer
    ...
    if loss_type == 'mse':
        ...
        if reduction == 'sum':
            ...
            recon_error = recon_error.mean() / x.size()[1] # if 2 channels
    elif loss_type == 'l1':
        recon_error = F.l1_loss(recon_x, x, reduction=reduction)
    elif loss_type == 'bce':
        #recon_error = F.binary_cross_entropy(recon_x, x, reduction=reduction)
        recon_error = F.binary_cross_entropy(recon_x.detach(), x.detach(), reduction=reduction)
    else:
        raise NotImplementedError
