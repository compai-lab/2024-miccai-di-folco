...
from torch.optim.lr_scheduler import ExponentialLR, CosineAnnealingLR, ReduceLROnPlateau, MultiStepLR
...
class Trainer:
    def __init__(self, training_params, model, data, device, log_wandb=True):
        ...
        if len(self.training_params['input_size']) == 2:
            ...
            input_size = (nc, self.training_params['input_size'][0])
        else:
            pass
        ...
        elif lr_sch_type == 'multistep':
            milestones = [0.5 * training_params['nr_epochs'], 0.75 * training_params['nr_epochs']]
